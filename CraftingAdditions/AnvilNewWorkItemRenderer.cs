...
{
    public class AnvilNewWorkItemRenderer : IRenderer
    {
        ...
        private BEAnvilNew beAnvil;
        Vec4f glowRgb = new Vec4f();

        public AnvilNewWorkItemRenderer(BEAnvilNew beAnvil, BlockPos pos, ICoreClientAPI capi)
        {
            this.pos = pos;
            this.api = capi;
        ...
        private void RenderRecipeOutLine()
        {
            ...
            outLineColorMul.A =
                1 - GameMath.Clamp((float)Math.Sqrt(plrPos.SquareDistanceTo(pos.X, pos.Y, pos.Z)) / 5 - 1f, 0, 1);
        ...
        public void RegenMesh(ItemStack ingot, byte[,,] voxels, bool[,,] recipeToOutlineVoxels)
        {
            ...
            if (beAnvil.IsIronBloom)
            {
                ...
                tposMetal = api.BlockTextureAtlas.GetPosition(api.World.GetBlock(new AssetLocation("ingotpile")),
                    "iron");
            }
            else
            {
                tposMetal = api.BlockTextureAtlas.GetPosition(api.World.GetBlock(new AssetLocation("ingotpile")),
                    ingot.Collectible.LastCodePart());
                tposSlag = tposMetal;
            }

            MeshData metalVoxelMesh =
                CubeMeshUtil.GetCubeOnlyScaleXyz(1 / 32f, 1 / 32f, new Vec3f(1 / 32f, 1 / 32f, 1 / 32f));
            ...
            for (int x = 0; x < 16; x++)
            {
                for (int y = 0; y < 6; y++)
                {
                    for (int z = 0; z < 16; z++)
                    {
                        ...
                        {
                            byte glowSub =
                                (byte)GameMath.Clamp(10 * (Math.Abs(x - 8) + Math.Abs(z - 8) + Math.Abs(y - 2)), 100,
                                    250);
            ...
            for (int x = 0; x < 16; x++)
            {
                for (int y = 0; y < 6; y++)
                {
                    for (int z = 0; z < 16; z++)
                    {
                        ...
                        voxelMeshOffset.Rgba = (requireMetalHere && mat == EnumVoxelMaterial.Empty)
                            ? greenVoxelMesh.Rgba
                            : orangeVoxelMesh.Rgba;
